...
import { getFriendRequest, acceptFriendReq } from "../lib/api";
...
const NotificationPage = () => {
  ...
  const { mutate: acceptFriendRequest, isPending } = useMutation({
    ...
    onSuccess: () => {
      querClient.invalidateQueries({ queryKey: ["friendReqs"] });
      querClient.invalidateQueries({ queryKey: ["friends"] });
      querClient.invalidateQueries({ queryKey: ["users"] });
      toast.success("Now you both are friends to each other");
    },
  });

  const incomingReq = friendReq?.incomingReq || [];
  const acceptReq = friendReq?.acceptReq || [];
  return (
              ...
              <section className="space-y-4">
                ...
                <div className="space-y-3">
                  {incomingReq.map((request) => (
                    <div
                      key={request._id}
                      className="card bg-base-200 shadow-md hover:shadow-md transition-shadow"
                    >
